...
def download_deeprec_resources(azure_container_url, data_path, remote_resource_name):
    ...
    """
    os.makedirs(data_path, exist_ok=True) # exist_ok：只有在目录不存在时创建目录，目录已存在时不会抛出异常
...
def cal_metric(labels, preds, metrics):
    ...
    for metric in metrics:
        if metric == "auc":
            # print('labels:', type(labels[0]), labels[:5])
            # print('preds:',type(preds[0]), preds[:5])
            # print('type_of_target(labels):', type_of_target(np.asarray(labels[0])))
            # print('type_of_target(preds):', type_of_target(np.asarray(preds[0])))
            # auc = roc_auc_score(np.concatenate((labels)), np.concatenate((preds))) # lstur
            auc = roc_auc_score(np.asarray(labels), np.asarray(preds)) # dkn
            res["auc"] = round(auc, 4)
        elif metric == "rmse":
            ...
            pred[pred >= 0.5] = 1
            pred[pred < 0.5] = 0
            f1 = f1_score(np.concatenate((labels)), pred)
            res["f1"] = round(f1, 4)
