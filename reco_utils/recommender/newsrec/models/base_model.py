...
class BaseModel:
    ...
    def run_news(self, news_filename):
        ...
        news_indexes = []
        news_vecs = []
        # { "news_index_batch": news_indexes_batch,"candidate_title_batch": candidate_title_index_batch, }
        for batch_data_input in tqdm(
    ...
    def run_fast_eval(self, news_filename, behaviors_file):
        news_vecs = self.run_news(news_filename) # dict(zip(news_indexes, news_vecs))
        user_vecs = self.run_user(news_filename, behaviors_file) # dict(zip(user_indexes, user_vecs))
